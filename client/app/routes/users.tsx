...
function UserList({ handleEditClick }: { handleEditClick: (userId: number) => void }) {
  ...
  function handleRemoveClick(e: any, name: string) {
    e.preventDefault()
      ...
      {users.map((user: any) => (
        <li key={user.id}>
          ...
          <p>
            ...
            <span> {user.role}</span>
            <span>
              <button onClick={e => handleRemoveClick(e, user.id)}>x</button>
              <button
                onClick={e => {
                  e.preventDefault()
...
export function meta({}: Route.MetaArgs) {
  return [{ title: 'Users' }, { name: 'description', content: 'User list of my app' }]
}
